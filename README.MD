...
自前ホストのカスタムエージェント（OSS LLM = Qwen 等）を、**Microsoft Agent 365 Skills** を使って **非 AI Teammate / OBO（委任）認可** のエージェントとして構築し、**Microsoft 365 管理センター**と **Microsoft Entra 管理センター**で「登録内容」と「ガバナンス（Kill Switch / 条件付きアクセス）」を実機で確認するまでを、一気通貫で学ぶ教材である。学習用の **BYO MCP（自作の外部 MCP サーバー）** も自分で建て、**Single Agent Map** に可視化されるところまで確認する。
...
本教材は「**第1部：環境構築**（作る）」→「**第2部：A365 ハンズオン**（管理センター/Entra で確認・統制する）」の 2 部で進む。

```text
━━━━━━━━━━ 第1部：環境構築 ━━━━━━━━━━        ━━━━━━━━━━ 第2部：A365 ハンズオン ━━━━━━━━━━
 ツール導入 ─▶ Skills 導入 ─▶ Blueprint/Agent ID      管理者承認(Requests) ─▶ Entra Agent ID 確認
     ─▶ アプリ実装(LLM/OBO/観測) ─▶ BYO MCP        ─▶ Agent Registry をタブ別に確認
     ─▶ ローカル確認 ─▶ デプロイ & 公開             ─▶ Single Agent Map で可視化
                                                    ─▶ ガバナンス（Block=Kill Switch / CA / 削除）
   （開発者 ＝ VS Code / ターミナル）                （AI 管理者 ＝ M365 管理センター / Entra 管理センター）
```
