...
> 💡 **①と②の関係**：**①＝エージェントの“頭脳”**（何を答え・どの道具を呼ぶか）、**②＝“受付と起動”**（外部からのメッセージを `/api/messages` で受け、認証して①に渡す Web サーバー部分）。両方そろって初めて動く。②は定型の“配管”（認証・ルーティング等）で `.env`・SDK に紐づくため、手書きせず Skills に自動生成させる。`app.py`（①）は②の `build_adapter` を import して②に差し込まれる。
