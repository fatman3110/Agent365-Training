...
- **第1部 B：独自エージェント（S2S）を作る** — 自前ホストの LLM（Qwen）を S2S（サービスプリンシパル）認可でフルコード実装し Azure にデプロイ。
...
flowchart LR
    subgraph P1["第1部：作成と申請 ｜ 開発者"]
        ...
        A0 -->|"B：フルコード"| AB["作業環境準備 → Blueprint（S2S） → 本体実装 → Azure デプロイ → 登録申請"]
    end
    subgraph P2["第2部：Agent 365 ハンズオン ｜ AI 管理者（M365 / Entra 管理センター）"]
        direction TB
        B1["承認して管理下に置く<br/>（A/B 共通）"] --> B2["エージェントを動かす<br/>（観測データを作る）"]
        B2 --> B3["Observe：観測"]
        B3 --> B4["Govern：管理"]
        B4 --> B5["Secure：保護"]
    end
...
| 部 | ゴール | 主な担当・画面 |
|----|--------|---------------|
| 第1部 作成と申請（A/B の 2 ルート） | エージェントを作って Agent 365 に登録申請（A: Copilot Studio ／B: 独自エージェント（S2S）） | 開発者：Copilot Studio ／ VS Code・Azure |
...
- **B：独自エージェント（S2S・フルコード）** → [docs/part1b-custom-agent.md](./docs/part1b-custom-agent.md)
...
| Microsoft Teams ライセンス | 公開したエージェントを Teams で使う（作成者・利用者の双方） | **A** | Teams ライセンス・Microsoft 365 Copilot |
| Azure サブスクリプション | App Service（+ ACR / Functions）でホスト| **B** | 従量課金（本トレーニング期間のみの利用であれば数ドル程度） |
...
| Azure へのデプロイ（App Service / ACR） | **B** | Azure RBAC: **Contributor**（リソース作成）/ **User Access Administrator**（RBAC 付与） |  |
...
| Agent Registry / Single Agent Map の閲覧 | A・B | **AI Reader** 以上 |  |
| M365 管理センターで件数（Activity）を閲覧 | A・B | **AI Reader** 以上 | Activity タブ詳細の表示に **E7 または Agent 365** ライセンスが必要 |
| Entra サインインログ（Agent）を閲覧 | A・B |  **Reports Reader** 以上 |  |
| Purview DSPM  で対話本文（Prompt/Response）を閲覧 | A・B | Information Protection Investigators 相当（Data Classification Content Viewer を含む） |  |
| Defender 高度なハンティング（CloudAppEvents）を実行 | A・B | **Security Reader**（閲覧）/ Security Operator |  |
...
| カスタムポリシーテンプレートの前提ポリシー作成（Entra） | A・B | 条件付きアクセス→**Conditional Access Administrator**<br>アクセスパッケージ→**Identity Governance Administrator**<br>カスタムセキュリティ属性→**Attribute Definition Administrator** | 特に、**Global 管理者でもカスタムセキュリティ属性の権限は既定で持たない**ため別途付与が必要 |
