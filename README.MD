...
2. 🧩 **エージェントに「道具（ツール）」を持たせる** — エージェントが「**どこと通信し・何を実行するか**」を把握するには**道具（MCP）**の把握が不可欠です。ここでは自作の小さな道具（`echo` / `now`）を作って**登録**します。
...
flowchart LR
    subgraph P1["第1部：環境構築 ｜ 開発者（VS Code / ターミナル）"]
        direction TB
        A1["作業端末環境構築"] --> A2["Skills 導入"] --> A3["Blueprint / Agent ID 作成"]
