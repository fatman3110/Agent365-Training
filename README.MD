...
3. **【第1部】本体を実装してクラウドにデプロイする** — ロジックを実装し、Azure にデプロイします。
4. **【第1部】Agent 365 に登録する** — エージェントと道具（MCP）を登録します。
5. **【第2部】エージェントを承認して管理下に置く** — 管理者がエージェントの組織としての利用を承認します。
6. **【第2部】管理者として統制する** — 管理者がエージェントを **ワンクリックで止める（Kill Switch）** など、ガバナンス（統制）が効くことを確認します。
...
flowchart LR
    ...
    subgraph P2["第2部：Agent 365 ハンズオン ｜ AI 管理者（M365 / Entra 管理センター）"]
        direction TB
        B1["エージェントの承認"] --> B2["Entra Agent ID 確認"]
        B2 --> B3["Agent Registry を確認"] --> B4["Single Agent Map で可視化"]
        B4 --> B5["観測を4画面で追う<br/>M365→Entra→Purview→Defender"]
        B5 --> B6["ガバナンスを検証<br/>棚卸し/Block/CA/削除"]
    end
    A6 ==> B1
```
