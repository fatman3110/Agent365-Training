...
> - **何に使う**：付けたタグを条件に、**きめ細かいアクセス制御（Azure ABAC）や、フィルタ・レポート・棚卸し**ができる。エージェントを「本番／検証」「部門」などで分類し、運用の絞り込み等に活かせる。
...
> **本番／開発で値を出し分けたいとき（重要）**：1 つのテンプレートが付与する属性値は**そのテンプレート単位で固定（静的）**であり、同じテンプレートを当てたエージェントは全部同じ値になる。またテンプレートは**新規アクティブ化のみ**に効き、承認済みエージェントには後から効かない（[Learn](https://learn.microsoft.com/microsoft-agent-365/admin/policy-template)）。したがって環境ごとに値を変えたいなら次のどちらかを採る。
> - **環境別にテンプレートを分ける**：本番用テンプレート（`Environment=Production`）と開発用テンプレート（`Environment=Development`）を別々に作り、エージェントのアクティブ化時に該当テンプレートを選ぶ。
> - **個別に手動割り当てする**：上記の手動付与手順で、エージェントごとに任意の値を付ける（既存の承認済みエージェントに付け直す場合もこちら）。
>
> どちらの場合も、属性値そのものは**静的なメタデータ**であり、「動的な制御」はこの値を**条件に使う側（条件付きアクセスやフィルタ）**で表現する、という役割分担になる。
