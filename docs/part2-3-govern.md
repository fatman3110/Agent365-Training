...
> **なぜ Select agents なのか**：テンプレート（5-2）の「追加するポリシー」一覧に出る CA は、**特定のエージェントを対象にしたもの**に限られる。対象化を **すべてのエージェント ID**（テナント全体一律）にすると、テンプレートの CA 一覧には**出ない**。テナント全体へ一律に効かせたい（テンプレートに束ねない）用途なら「すべてのエージェント ID」でもよい。
...
「エージェントに空のグループのメンバー権を渡す」構成。グループ自体に権限を与えなければ影響はないが、アクセスパッケージの仕組み（**カタログ → リソース → 付与**）を一通り体験できる。
...
4. **アクセス パッケージを作る**：**エンタイトルメント管理 › アクセス パッケージ › + 新しいアクセス パッケージ**
   - **基本**：名前（例 `Agent-Training-Package`）／カタログ＝`Agent-Training-Catalog`
   - **リソース ロール**：`Agent-Training-Group` を追加し、ロール＝**メンバー**
