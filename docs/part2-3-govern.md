...
Block は「一時停止」。学習を終えたら、エージェント本体（Blueprint と Agent ID）と自前ホストの Azure リソースまで消す。
...
このハンズオン（自前ホスト・非 AI Teammate）の片付け：

1. 作業ディレクトリで `a365 cleanup`（**破壊的**。config の Blueprint 配下＝Blueprint と Agent ID、関連する Entra アプリ登録を一括削除）
2. 取り残し（orphan）確認：`az ad app list --display-name "<blueprint名>" -o table` → 残っていれば `az ad app delete --id <appId>`
3. Azure リソースを削除：App Service / Functions / ACR / ストレージをまとめて `az group delete -n $RG`

> ⚠️ **後片づけ必須**：Block（一時停止）のまま放置しない。連鎖クリーンアップは非同期で数時間〜数日かかることがある。
