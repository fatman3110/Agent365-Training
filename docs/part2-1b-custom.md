...
> - **カスタムテンプレート**を使うなら、**先に Entra でポリシーを作成**しておく必要がある（未作成だとテンプレート作成時に選べない）。CA・アクセスパッケージ・カスタムセキュリティ属性の**フル手順**は [Secure：カスタムポリシーテンプレートを作る](./part2-4-secure.md#5-カスタムポリシーテンプレートを作る) を参照。作成後、テンプレートに束ねてこのウィザードで選ぶ。
